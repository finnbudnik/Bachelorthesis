        ...
        for _, row in period_actions[period_actions['action'] == 'Sell'].iterrows():
            ...
            for lot in portfolio[stock]:
                ...
                if lot_amount > sell_now:
                    updated_lots.append([lot_period, lot_amount - sell_now, lot_price])
                remaining -= sell_now

            if updated_lots:
                portfolio[stock] = updated_lots
            else:
                del portfolio[stock] 
...
result_df['PGR'] = result_df['real_gain'] / (
    result_df['real_gain'] + result_df['pot_gain']
)
result_df['PLR'] = result_df['real_loss'] / (
    result_df['real_loss'] + result_df['pot_loss']
)

result_df['PGR'] = result_df['PGR'].fillna(0)
result_df['PLR'] = result_df['PLR'].fillna(0)

result_df.to_csv("transdata_de_fifo.csv", index=False)
